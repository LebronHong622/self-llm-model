...
class SelfGPTModel(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.config = config
        # 嵌入编码 + 绝对位置编码
        self.embedding = InputEmbeddingLayer(config.vocab_size, config.hidden_size, config.context_length)
        ...
        # 归一化层
        self.norm_layer = nn.LayerNorm(config.hidden_size, config.norm_eps)
